/*
    ...
    [V0.12 修改] 電量 < 5% 停止量測；>= 10% 持續通過穩定時間後重啟。
...
void setup(void) {
    ...
    /* [修改] 開機低電或 ADC 故障時直接顯示對應警示，不播放版本畫面、不啟動量測中斷。 */
    brd_oled_begin();

    /* [新增] 版本畫面等待結束後再次更新 ADC，避免低電時仍啟動量測。 */
    brd_battery_update();
    /* [V0.12 修改] 低電鎖定或 ADC 無有效資料時暫停；刪減僅檢查低電的條件。 */
...
void loop(void) {
    ...
    if (!brd_battery_measurement_allowed()) {
        ...
        if (brd_battery_restart_required()) {
            /* [V0.12 修改] >= 10% 持續通過 cfg 恢復時間後重新開機。 */
            esp_restart();
        }
        brd_oled_update();
    ...
    /* [V0.12 新增] ADC 故障恢復後重新待測；已啟用時此呼叫不重置資料。 */
    brd_measurement_begin();
